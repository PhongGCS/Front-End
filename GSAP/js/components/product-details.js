...
export class ProductDetails extends Component {
  ...
  updateTooltipLine(detailItem) {
    ...
    const angleDeg = angleValue.endsWith('deg') ? Number.parseFloat(angleValue) : 0
    ...
    const usePinVars = Number.isFinite(pinXFromVar) && Number.isFinite(pinYFromVar)
  ...
  updateTooltipLines() {
    this.detailSequence?.forEach((detailItem) => this.updateTooltipLine(detailItem))
  }
  ...
  setupTooltipLines() {
    ...
    ScrollTrigger.addEventListener('refresh', () => this.scheduleTooltipUpdate())
    ...
    detailItems.forEach((detailItem) => {
      ...
      img.addEventListener('load', () => this.scheduleTooltipUpdate(), { once: true })
      img.addEventListener('error', () => this.scheduleTooltipUpdate(), { once: true })
    })
  }
}
